...
func (d *VersionDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	// Create a copy of the Id for when not found during getAndSetVersionAttributes
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError(
			"Failed to read nd_version data source",
			fmt.Sprintf("The nd_version data source with id '%s' has not been found", cachedId),
		)
		return
	}
