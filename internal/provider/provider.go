...
type ndProviderModel struct {
	...
	// Below attributes may not be required.
...
// func (p *ndProvider) Schema(_ context.Context, _ provider.SchemaRequest, resp *provider.SchemaResponse) {
	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			"username": schema.StringAttribute{
				Description: "Username for the APIC Account. This can also be set as the ND_USERNAME environment variable.",
				Optional:    true,
			},
			"password": schema.StringAttribute{
				Description: "Password for the APIC Account. This can also be set as the ND_PASSWORD environment variable.",
				Optional:    true,
			},
			...
			"annotation": schema.StringAttribute{
				Description: "Global annotation for the provider. This can also be set as the ND_ANNOTATION environment variable.",
				Optional:    true,
			},
		},
	}
...
func (p *ndProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
	...
	// validateRelationDn := stringToBool(resp, "insecure", getStringAttribute(data.ValidateRelationDn, "ND_VAL_REL_DN"), true)
	// maxRetries := stringToInt(resp, "retries", getStringAttribute(data.MaxRetries, "ND_RETRIES"), 2)
	// setGlobalAnnotation(data.Annotation, "ND_ANNOTATION")
	...
	if password != "" {
		ndClient = client.GetClient(url, username, client.Password(password), client.Insecure(isInsecure), client.ProxyUrl(proxyUrl), client.Domain(domain), client.Platform(platform))
	} else {
		ndClient = nil
	}
